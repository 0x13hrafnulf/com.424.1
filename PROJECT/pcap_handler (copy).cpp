    ...
    else
    {
      if(((it->second).ip_dst.s_addr != ipHeader->ip_dst.s_addr) or (it->second.source_port != udpHeader->source) or (it->second.protocol != IPPROTO_UDP))
      {
        if(it->second.next == nullptr){
          ...
          {
            if(n_ptr->ip_dst == ipHeader->ip_dst.s_addr)
            {

            }
            else if(n_ptr->source_port == udpHeader->source)
            {

            }
            else if(n_ptr->protocol == udpHeader->source)
            {
          ...
          {
            it->second.next->port_count += 1;
            it->second.next->scan_weight += udpHeader->dest < 1024 ? 3 : 1;
            it->second.next->scan_delay_avg += now - it->second.time_update;
            it->second.next->time_update = now;
          }
      ...
      {
        if((it->second.ports.insert(udpHeader->dest)).second == true)
        {
          it->second.port_count += 1;
          it->second.scan_weight += udpHeader->dest < 1024 ? 3 : 1;
          it->second.scan_delay_avg += now - it->second.time_update;
          it->second.time_update = now;
        }
      }
    }
    ...
    else
    {
      if(((it->second).ip_dst.s_addr != ipHeader->ip_dst.s_addr) or (it->second.source_port != tcpHeader->th_sport) or (it->second.protocol != IPPROTO_TCP))
      {
        if(it->second.next == nullptr){
          ...
          if((it->second.next->ports.insert(tcpHeader->th_dport)).second == true)
          {
            it->second.next->port_count += 1;
            it->second.next->scan_weight += tcpHeader->th_dport < 1024 ? 3 : 1;
            it->second.next->scan_delay_avg += now - it->second.time_update;
            it->second.next->time_update = now;
            it->second.next->protocol = IPPROTO_TCP;
            if(it->second.next->source_port != s_port) it->second.next->flags |= PORT_CHANGE;
          }
        }
      }
      else
      {
        if((it->second.ports.insert(tcpHeader->th_dport)).second == true)
        {
          it->second.port_count += 1;
          it->second.scan_weight += tcpHeader->th_dport < 1024 ? 3 : 1;
          it->second.scan_delay_avg += now - it->second.time_update;
          it->second.time_update = now;
          it->second.protocol = IPPROTO_TCP;
          if(it->second.source_port != s_port) it->second.flags |= PORT_CHANGE;
        }
      }
    }
  }
